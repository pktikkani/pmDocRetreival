    ...
    def __init__(
        ...
        **kwargs,
        ...
        if (
            ...
            and "colqwen2_5" not in pretrained_model_name_or_path.lower()
            and "colnomic" not in pretrained_model_name_or_path.lower()
        ):
            raise ValueError(
                "This pre-release version of Byaldi only supports ColPali, ColQwen2 and ColQwen2_5 for now. Incorrect model name specified."
            )
        ...
        self.doc_ids = set()
        # First, initialize the model variable
        self.model = None

        # Ensure we're not trying to use CUDA if it's not available
        if device == "cuda" and not torch.cuda.is_available():
            print("CUDA requested but not available. Falling back to MPS or CPU.")
        ...
        elif "colqwen2_5" in pretrained_model_name_or_path.lower():
            self.model = ColQwen2_5.from_pretrained(
                self.pretrained_model_name_or_path,
                torch_dtype=torch.float16,
                device_map="cpu",
        ...
        # Make sure model was initialized
        if self.model is None:
            raise ValueError(f"Could not initialize model for {pretrained_model_name_or_path}")

        # Set model to eval mode
        self.model = self.model.eval()

        # Processor initialization...
        # Initialize appropriate processor based on model type
        if "colpali" in pretrained_model_name_or_path.lower():
        ...
        elif "colnomic" in pretrained_model_name_or_path.lower() or "colqwen2_5" in pretrained_model_name_or_path.lower():
        ...
        if self.device == "mps" and torch.backends.mps.is_available():
            ...
            # Fallback
            print(f"Fallback: using model on CPU")
            self.device = "cpu"
